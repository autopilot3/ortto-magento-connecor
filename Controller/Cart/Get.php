...
class Get extends AbstractJsonController implements HttpGetActionInterface
{
    ...
    public function __construct(
        ...
        // NOTE: Magento\Checkout\Model\Session's getQuote returns null when caching is enabled
        // https://github.com/magento/magento2/issues/6624
        // Using `Cart` until we find a workaround
        Cart $cart
    ) {
    ...
    public function execute()
    {
        try {
            ...
            if ($cartData->load($this->cart->getQuote())) {
                $payload = [
